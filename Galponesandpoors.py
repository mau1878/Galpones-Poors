...
# Function to fetch the close price for a given ticker and date, or the previous available date
def fetch_close_price(ticker, date):
    end_date = date + timedelta(days=1)  # Extend search to next day to include selected date
    for i in range(5):  # Check up to 5 days before the selected date if no data is found
        df = yf.download(ticker, start=date - timedelta(days=i + 1), end=end_date)['Close']
        if not df.empty:
            return df.dropna().iloc[-1]  # Return the last valid close price found
    return None
